...
def modelAnalysis(model_path,config_path,cluster_model_path,device,enhance,diff_model_path,diff_config_path,only_diffusion,use_spk_mix,local_model_enabled,local_model_selection):
    ...
    try:
        ...
        for i in spks:
            msg += i + " "
        print(f"modelAnalysis  choices={spks} {model.spk2id.keys()}")
...
def vc_fn(sid, input_audio, output_format, vc_transform, auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,f0_predictor,enhancer_adaptive_key,cr_threshold,k_step,use_spk_mix,second_encoding,loudness_envelope_adjustment):
    ...
    try:
        ...
        print(f"vc_fn 音频转换接口 input_audio={input_audio} truncated_basename={truncated_basename}")
        ...
        with gr.TabItem("推理"):
            ...
            with gr.Row(variant="panel"):
                with gr.Column():
                    ...
                    with gr.Tabs():
                        ...
                        with gr.TabItem('本地') as local_model_tab_local:
                            ...
                            local_choices=scan_local_models()
                            local_model_selection = gr.Dropdown(label='选择模型文件夹', choices=local_choices, value=local_choices[0], interactive=True)
                    with gr.Row():
                ...
                with gr.Column():
                    ...
                    sid = gr.Radio(choices=[], label="音色（说话人）")
            ...
            with gr.Row(variant="panel"):
                ...
                with gr.Column():
                    ...
                    cl_num = gr.Number(label="音频自动切片，0为不切片，单位为秒(s)", value=20)
